    ...
    def startStream(self):
        ...
        if not self.ffmpeg:
            ...
            try:
                ...
                ffmpeg_cmd = '{FFMPEG} -re -f mjpeg -framerate {framerate} -i {webcam_stream} -ar 44100 -ac 2 -acodec ' \
                             'pcm_s16le -f s16le -ac 2 -i /dev/zero -acodec aac -ab 128k -strict experimental -s ' \
                             '{stream_resolution} -vcodec h264 -pix_fmt yuv420p -g 10 -vb 700k -framerate {framerate} ' \
                             '-f flv -filter:v {filters} {rtmp_stream_url}'.format(FFMPEG,
                                                                                   webcam_stream,
                                                                                   stream_resolution,
                                                                                   framerate,
                                                                                   filters,
